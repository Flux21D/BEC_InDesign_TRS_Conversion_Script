...
var LessonFrameReport = 'Doc Name,Lesson Frame Count,ParaStyleCount, Lesson Frame Founded pages';
var EmptyFrameReport='Doc Name,EmptyFram1e Count,Pages';
var AllLabelFrameReport='DocName,PageName,TextFrameLabelName,Content'
var LessonStyleReport='DocName,PageName,FrameName';
var LessonStyleIssueReport='DocName,PageName,FrameName,ContentLength';
var aag_bbFrameReport='DocName,Ungrouping/Missing PageName';
var unwantedFrameListReport='DocName,PageName';
...
if (myFolders != null ) {
     if(myFolders.toString().match(GradeRegex) != null) {
        ...
        var assetReportFile = new File(reportFolder + '/BEC-TRS_LessonFrameReport.csv');
        assetReportFile.open('w');
        assetReportFile.writeln(LessonFrameReport);
        assetReportFile.close();
        
        var EmptyFrameReportFile=new File(reportFolder + '/BEC-TRS_EmptyFrameReport.csv');
        EmptyFrameReportFile.open('w');
        EmptyFrameReportFile.writeln(EmptyFrameReport);
        EmptyFrameReportFile.close();
        
          var AllTextFrameReportFile=new File(reportFolder + '/BEC-TRS_AllTextFrameReport.csv');
        AllTextFrameReportFile.open('w');
        AllTextFrameReportFile.writeln(AllLabelFrameReport);
        AllTextFrameReportFile.close();    
        
        var LessonStyleReportFile=new File(reportFolder + '/BEC-LessonStyleReport.csv');
        LessonStyleReportFile.open('w');
        LessonStyleReportFile.writeln(LessonStyleReport);
        LessonStyleReportFile.close();        
        
        var LessonIssueStyleReportFile=new File(reportFolder + '/BEC-LessonStyleIssueReport.csv');
        LessonIssueStyleReportFile.open('w');
        LessonIssueStyleReportFile.writeln(LessonStyleIssueReport);
        LessonIssueStyleReportFile.close();
        
        var unwantedFrameListFile=new File(reportFolder + '/BEC-unwantedFrameList.csv');
        unwantedFrameListFile.open('w');
        unwantedFrameListFile.writeln(unwantedFrameListReport);
        unwantedFrameListFile.close();         
        
        var aag_bbFrameReportFile=new File(reportFolder + '/BEC-Missing_AAG_bb_FrameReport.csv');
        aag_bbFrameReportFile.open('w');
        aag_bbFrameReportFile.writeln(aag_bbFrameReport);
        aag_bbFrameReportFile.close();
        ...
        for(var i=0; i<GradeFolder.length; i++){
            ...
            var assetReportFile = new File(reportFolder + '/BEC_TRS_FrameList.csv');
            assetReportFile.open('w');
 ...
 /*PageItems Array Start*/
    for(var PI=0;PI<pages[p].allPageItems.length;PI++)
    {
        if(pages[p].allPageItems[PI] instanceof TextFrame){
              if(pages[p].allPageItems[PI].associatedXMLElement == null){
                if(pages[p].allPageItems[PI].label == '' || pages[p].allPageItems[PI].label == undefined){
                    unwantedFrameListReport = unwantedFrameListReport + '\n' + app.activeDocument.name + ',' + pages[p].name;
                }
            }
        }
